...
def generate_tool_page(tool: dict, entries: List[ReleaseEntry]) -> str:
    ...
    lines = [
        ...
        f"| Secret Detection | {_feature_mark(tool, 'secret_detection')} |",
        f"| License Scanning | {_feature_mark(tool, 'license_detection')} |",
        f"| Build Tool Plugin | {_feature_mark(tool, 'build_tool_plugin')} |",
        f"| API Server | {_feature_mark(tool, 'api_server')} |",
        f"| Agentless SSH Scanning | {_feature_mark(tool, 'agentless_ssh')} |",
...
def generate_tool_page_ja(tool: dict, entries: List[ReleaseEntry]) -> str:
    ...
    lines = [
        ...
        f"| シークレット検出 | {_feature_mark(tool, 'secret_detection')} |",
        f"| ライセンススキャン | {_feature_mark(tool, 'license_detection')} |",
        f"| ビルドツールプラグイン | {_feature_mark(tool, 'build_tool_plugin')} |",
        f"| APIサーバー | {_feature_mark(tool, 'api_server')} |",
        f"| SSH無エージェントスキャン | {_feature_mark(tool, 'agentless_ssh')} |",
