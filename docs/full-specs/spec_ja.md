...
- 基本情報テーブルの費用表示：`pricing_url` が設定され `pricing` 文字列に `Paid`（大文字小文字は区別しない）が含まれる場合、`Paid` 部分を `pricing_url` へのリンクとして表示する（例：`Free / [Paid](...)`）
- **機能表**：12 項目の機能フラグを ✅/❌ で表示
  - コンテナスキャン、言語/ライブラリスキャン、SBOM生成、ポリシー評価、IaCスキャン、シークレット検出、ライセンススキャン、ビルドツールプラグイン、APIサーバー、SSH無エージェントスキャン、ダッシュボード、集中管理
...
- **概要版テーブル**：ツール名（個別ページへのリンクと機能一覧リンクを含む）・最新バージョン・最終更新日・種別・ライセンス・費用および基本機能フラグ（コンテナ・言語/ライブラリ・SBOM・ポリシー）。費用列は個別ページと同じ `pricing_url` + `Paid` 部分リンク化ルールで表示する。
- **詳細版テーブル**：ツール名（個別ページへのリンクと機能一覧リンクを含む）・全機能フラグ・独自機能列を含む。機能フラグ一覧：
  - コンテナ、言語/ライブラリ、SBOM、ポリシー、IaC、シークレット検出、ライセンススキャン、ビルドプラグイン、APIサーバー、SSH無エージェント、ダッシュボード、集中管理
